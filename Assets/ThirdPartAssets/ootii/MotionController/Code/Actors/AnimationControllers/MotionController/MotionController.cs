        ...
        public void OnControllerLateUpdate(ICharacterController rController, float rDeltaTime, int rUpdateIndex)
        {
            ...
            if (mAnimator != null)
            {
                ...
                for (int i = 0; i < lCount; i++)
                {
                    ...
                    if (mState.AnimatorStates[i].AutoClearMotionPhase && lActiveMotion != null)
                    {
                    ...
                    if (lActiveMotion != null && !lActiveMotion.HasAutoGeneratedCode)
                    {
                        ...
                        if (mState.AnimatorStates[i].StateInfo.fullPathHash != mPrevState.AnimatorStates[i].StateInfo.fullPathHash)
                        {
                            ...
                            if (lActiveMotion.IsMotionState(mState.AnimatorStates[i].StateInfo.fullPathHash))
                            {
                                lActiveMotion.IsAnimatorActive = true;
                                mState.AnimatorStates[i].AutoClearMotionPhaseReady = true;
                            }
            ...
            {
                Log.FileScreenWrite(String.Format("{0} MC.Update() Motion:{1} MotionDur:{2:f4} Phase:{3} Param:{4} State:{5}", name, (GetActiveMotion(0) != null ? GetActiveMotion(0).GetType().Name : "null"), (GetActiveMotion(0) != null ? GetActiveMotion(0).Age : 0), mState.AnimatorStates[0].MotionPhase, mState.AnimatorStates[0].MotionParameter, AnimatorHashToString(mState.AnimatorStates[0].StateInfo, mState.AnimatorStates[0].TransitionInfo)), 3);

                if (_CameraTransform != null)
                {
                    float lCameraFromAvatar = NumberHelper.GetHorizontalAngle(_Transform.forward, _CameraTransform.forward);
                    Log.FileScreenWrite(String.Format("CFA:{0:f3} iX:{1:f3} iY:{2:f3}", lCameraFromAvatar, mState.InputX, mState.InputY), 4);
                }
