...
namespace com.ootii.Actors.AnimationControllers
{
    ...
    public abstract class MotionControllerMotion : BaseObject
    {
        ...
        protected virtual void GenerateScripts(AnimatorController rAnimatorController, int rAnimatorLayer, string rAnimatorSMName)
        {
            ...
            {
                lText.AppendLine("private AnimationClip m" + CleanName(mAnimationClipIDs[i].ToString()) + " = null;");
            }
            ...
            for (int i = 0; i < mAnimationClipIDs.Count; i++)
            {
                ...
                string lClipName = "m" + CleanName(mAnimationClipIDs[i].ToString());
        ...
        /// <param name="rBlendTree"></param>
        /// <param name="rText"></param>
        private void CreateBlendTree(string rParentID, BlendTree rBlendTree, StringBuilder rText, bool rAddAsChild = false)
        {
            ...
            for (int i = 0; i < rBlendTree.children.Length; i++)
            {
                ...
                if (rBlendTree.blendType == BlendTreeType.Simple1D)
                {
                    ...
                    {
                        CreateBlendTree(lBlendTreeID, rBlendTree.children[i].motion as BlendTree, rText, true);
                    }
                }
                ...
                else
                {
                    ...
                    {
                        rText.AppendLine("    " + lBlendTreeID + ".AddChild(m" + CleanName(lChild.motion.GetInstanceID().ToString()) + ", " + lChild.threshold + "f);");
                    }
                    else if (lChild.motion is BlendTree)
                    {
                        CreateBlendTree(lBlendTreeID, rBlendTree.children[i].motion as BlendTree, rText, true);
