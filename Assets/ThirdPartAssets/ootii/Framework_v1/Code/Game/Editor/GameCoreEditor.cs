...
public class GameCoreEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUI.BeginChangeCheck();
        lBool = EditorGUILayout.Toggle(new GUIContent("Is Cursor Visible", "Determines if the mouse cursor starts as visible."), mTarget.IsCursorVisible);
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(target, "Set Is Cursor Visible");
            mTarget.IsCursorVisible = lBool;
        }

        EditorGUI.BeginChangeCheck();
        string lString = EditorGUILayout.TextField(new GUIContent("Cursor Alias", "Action alias to determine if the cursor should be visible."), mTarget.ShowCursorAlias);
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(target, "Set Cursor Alias");
            mTarget.ShowCursorAlias = lString;
        }
